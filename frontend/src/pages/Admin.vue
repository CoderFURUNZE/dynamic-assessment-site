...
const activeTab = computed({
  get: () => {
    if (route.path.startsWith("/admin/")) {
      ...
      if (["video", "questions", "kps", "edges", "users", "report", "expression"].includes(seg)) return seg;
    }
    return "config";
  },
  set: (value) => {
...
<template>
  <el-card v-if="isPrivileged" class="panel-card" shadow="never">
    <div style="display: flex; flex-wrap: wrap; gap: 8px; align-items: center; justify-content: space-between">
      <div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap">
        <el-select v-model="subject" size="small" style="width: 180px" @change="() => { loadConfig(); loadKps(); }">
  ...
  <el-tabs v-model="activeTab" type="border-card">
    ...
    <el-tab-pane label="视频绑定" name="video">
      <el-card shadow="never">
        <template #header>知识点绑定B站选集</template>
      ...
      <el-card shadow="never">
        ...
        <el-form label-width="120px" size="small">
          ...
          <el-form-item label="自托管链接">
            <el-input v-model="localVideoUrl" placeholder="可填写 mp4 或 m3u8 地址" />
          </el-form-item>
          <el-form-item>
