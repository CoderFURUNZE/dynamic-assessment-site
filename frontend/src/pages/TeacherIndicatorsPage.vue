...
function syncQuery() { saveTeacherSubject(subject.value); router.replace({ path: "/teacher/evaluation", query: { ...buildTeacherSubjectQuery(subject.value), tab: "indicators" } }); }
...
<template>
  <div class="teacher-page">
    <WorkspaceTopbar v-model="subject" :courses="courses" badge="Teacher Indicators" title="评价内容选择" @change="syncQuery">
      <el-button @click="router.push({ path: '/teacher/students', query: { subject: subject || undefined, tab: 'class' } })">看班级结果</el-button>
    </WorkspaceTopbar>
