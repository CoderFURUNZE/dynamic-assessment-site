...
function syncQuery() { saveTeacherSubject(subject.value); router.replace({ path: "/teacher/students", query: buildTeacherSubjectQuery(subject.value, { user_id: selectedStudentId.value ? String(selectedStudentId.value) : undefined }) }); }
...
<template>
  <div class="teacher-page">
    <WorkspaceTopbar v-model="subject" :courses="courses" badge="Teacher Students" title="学生详情" @change="syncQuery">
      <el-button @click="router.push({ path: '/teacher/analytics', query: { subject: subject || undefined } })">返回课程分析</el-button>
    </WorkspaceTopbar>
